...
export default function ChatPanel({ node, onClose, onNodeSpawned }) {
  ...
  async function handleSpawn(title) {
    try {
      ...
      setShowSpawn(false);
      if (onNodeSpawned) onNodeSpawned();
    } catch (err) {
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        gap: '14px',
        flexShrink: 0,
        background: 'linear-gradient(to bottom, rgba(10,10,15,0.98), rgba(10,10,15,0.92))',
      }}>
        <button
          onClick={onClose}
          style={{
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          <div style={{ fontSize: '16px', fontWeight: '600', color: '#f0f0f8', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>
            {node.title}
          </div>
          {provider && (
      ...
      <div style={{
        ...
        gap: '10px',
        flexShrink: 0,
        background: 'linear-gradient(to top, rgba(10,10,15,0.98), rgba(10,10,15,0.94))',
      }}>
        <textarea
          ref={inputRef}
          value={input}
